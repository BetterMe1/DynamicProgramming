...
class Solution {
    ...
    public boolean isPal(char[] chars,int j,int i) {
        if(i-j <2){
            return true;
        }
        return chars[j]==chars[i-1] && isPal(chars,j+1,i-1);
        */
/*
...
72. 编辑距离
 */
//状态：word1的前i个字符转成word2的前j个字符的编辑距离
//插入：F(i,j) = F(i,j-1)+1;
//删除：F(i,j) = F(i-1,j)+1;
//替换: F(i,j) = F(i-1,j-1)+ (word1[i]==word2[j] ? 0 :1 );
//初始：F(0,j) = j;
//      F(i,0) = i;
/*
class Solution {
...
115. 不同的子序列
 */
//状态：S的前i个字符构成的子序列这中出现T的前j个字符的个数
//F(i,j):S[i] == T[i] :使用 F(i-1,j-1) + 不使用 F(i-1,j)
//       S[i] != T[i] :F(i-1,j)
class Solution {
